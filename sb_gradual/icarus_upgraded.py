#!/usr/bin/env python3
"""
Upgraded switching bandit script for GRADUAL switch:
...
- Environment gradually shifts optimal arm from 0 to 1 over ANNEAL_EPISODES.
"""
...
def train(episodes: int = 20000, seed: int | None = SEED, **kwargs):
    ...
    for ep in range(1, episodes + 1):
        # Environment logic: Gradual annealing
        if ep <= SWITCH_EP:
            prob = [1.0, 0.0]
    ...
    plt.ylim(0, 105)
    plt.title("Gradual Switch Bandit - Decoupled Icarus Upgraded")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower right")
    
    out_dir = f"sb_gradual/runs/{seed}_icarus_upgraded" if seed is not None else "sb_gradual/runs/noseed_icarus_upgraded"
    ...
    if not quiet:
        print(f"\nPlot saved to '{png_path}' and CSV to '{csv_path}'")
        print(f"Max Critic Scale: {max_critic_scale:.3f}")
    
    return np.mean(reward_history[-1000:])
