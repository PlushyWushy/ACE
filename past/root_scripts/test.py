...
Actor LR follows a fixed schedule tied to the switch point.
"""
#TODO: Make ACh use its own fast-slow novelty on the variance estimate. Continuously rising lr is not allowing the model to adapt fast enough. 
import argparse
import math
import numpy as np
import random
...
def train(args):
    ...
    for ep in range(args.episodes):
        ...
        if ep < SWITCH_EPISODE:
            ...
            actor_lr = ACTOR_LR_PEAK + (ACTOR_LR_BASE - ACTOR_LR_PEAK) * frac
        else:
            actor_lr = ACTOR_LR_BASE
    ...
    ax1.axvline(x=SWITCH_EPISODE, color="r", linestyle="--", label="Switch Point")
    ...
    fig.tight_layout()
    plt.title("Switch CartPole - Decoupled Uncertainty (TD-LTP Critic, Actor LR Schedule)")
