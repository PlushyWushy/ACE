...
"""
#TODO: Make ACh use its own fast-slow novelty on the variance estimate.
import argparse
import math
import numpy as np
import random
...
def train(args):
    ...
    for ep in range(args.episodes):
        ...
        while not done:
            ...
            # Keep critic learning rate constant (no ACh modulation).
    ...
    fig.tight_layout()
    plt.title("Switch CartPole - Decoupled Uncertainty (TD-LTP Critic)")

    out_dir = f"cartpole/runs/{args.seed}_critic_ach_decoupled_tdlp" if args.seed is not None else "cartpole/runs/noseed_critic_ach_decoupled_tdlp"
    os.makedirs(out_dir, exist_ok=True)

    png_path = os.path.join(out_dir, f"{args.seed}_critic_ach_decoupled_tdlp.png")
    fig.savefig(png_path, dpi=150, bbox_inches="tight")

    csv_path = os.path.join(out_dir, f"{args.seed}_critic_ach_decoupled_tdlp.csv")
    with open(csv_path, "w") as fh:
        fh.write("episode,reward,unexpected_uncertainty,expected_uncertainty,mean_variance,mean_td_error_sq,mean_delta_var,mean_abs_td\n")
        for i, (r, u, e, v, td2, dv, td) in enumerate(zip(reward_history, unexpected_history, expected_history, variance_history, td2_history, delta_var_history, td_history)):
            fh.write(f"{i},{r},{u},{e},{v},{td2},{dv},{td}\n")
