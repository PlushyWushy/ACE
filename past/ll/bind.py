#!/usr/bin/env python3
"""
Monolithic TD-STDP LunarLander script with 16k place-cell input.

This is a single-file copy of the `td_stdp` system (main + util + bindsnet_add),
with an added 16,384-cell place-cell encoder for LunarLander and ADAM forced off.

Usage:
  python -u ll/td_stdp_ll_placecells.py
  python -u ll/td_stdp_ll_placecells.py -c config_ll_placecells_16k.ini
"""
...
# Disabled by default for LunarLander runs.
DEFAULT_SWITCH_AT = -1
...
class State_to_spike_PlaceCell_LunarLander16k:
    """
    16k place-cell population code for LunarLander:
      4 x 4 x 4 x 4 x 4 x 4 x 2 x 2 = 16384 cells over
      (x, y, x_dot, y_dot, theta, theta_dot, legL, legR).

    Ranges match `ll/flagship.py` and roughly align with `td_stdp/main.py` state limits.
    """
    ...
    def __init__(self, time, basis=False, rep=1, rho_pc=50.0, dt_s=1e-3):
        ...
        m0 = torch.linspace(-1.0, 1.0, 4)    # x
        m1 = torch.linspace(-0.3, 1.2, 4)    # y
        m2 = torch.linspace(-2.0, 2.0, 4)    # x_dot
        m3 = torch.linspace(-2.0, 2.0, 4)    # y_dot
        m4 = torch.linspace(-1.0, 1.0, 4)    # theta
        m5 = torch.linspace(-1.0, 1.0, 4)    # theta_dot
        m6 = torch.linspace(0.0, 1.0, 2)     # legL contact
        m7 = torch.linspace(0.0, 1.0, 2)     # legR contact

        mesh = torch.meshgrid(m0, m1, m2, m3, m4, m5, m6, m7, indexing="ij")
        centers = torch.stack([x.flatten() for x in mesh], dim=1)
        self.centers = centers  # [16384, 8]
        ...
        s4 = (m4[1] - m4[0]) / 1.5
        s5 = (m5[1] - m5[0]) / 1.5
        self.sigmas = torch.tensor([s0, s1, s2, s3, s4, s5, 0.5, 0.5]).unsqueeze(0)  # [1, 8]

        self._clip_min = torch.tensor([m0.min(), m1.min(), m2.min(), m3.min(), m4.min(), m5.min(), m6.min(), m7.min()])
        self._clip_max = torch.tensor([m0.max(), m1.max(), m2.max(), m3.max(), m4.max(), m5.max(), m6.max(), m7.max()])
    ...
    def __call__(self, state, augment=None):
        state_t = torch.as_tensor(state, dtype=torch.float32)
        if state_t.ndim != 2 or state_t.shape[1] != 8:
            raise ValueError(f"LunarLander place-cell encoder expects state shape [batch, 8], got {tuple(state_t.shape)}")

        state_t = torch.clamp(state_t, self._clip_min, self._clip_max)
        diff = (state_t.unsqueeze(1) - self.centers.unsqueeze(0)) ** 2  # [B, 16384, 8]
        exponent = -torch.sum(diff / (2 * (self.sigmas ** 2)), dim=2)  # [B, 16384]
        rates = self.rho_pc * torch.exp(exponent)  # Hz
        probs = 1.0 - torch.exp(-rates * self.dt_s)  # per-step spike prob
        probs = torch.clamp(probs, 0.0, 1.0)
        ...
        if self.basis:
            extras.append(torch.ones(self.time, bsz, self.rep, 1, dtype=torch.float32))
        if augment is not None:
            aug_t = torch.as_tensor(augment, dtype=torch.float32)  # [B, k]
            extras.append(aug_t.unsqueeze(0).unsqueeze(2).expand(self.time, bsz, self.rep, -1))
        if extras:
            spikes = torch.cat([spikes] + extras, dim=-1)
...
class batch_envs:
    def __init__(self, name, batch_size=1, rest_n=100, warm_n=100):
        ...
        try:
            ...
            msg = str(e)
            if ("Box2D" in msg) or ("box2d" in msg) or ("DependencyNotInstalled" in type(e).__name__):
                raise RuntimeError(
                    "Failed to create LunarLander environment because Box2D is missing. "
                    "Install with `pip install 'gym[box2d]'` (or `pip install 'gymnasium[box2d]'` if using gymnasium)."
                ) from e
            raise
...
class Snn_actor:
    ...
    def __call__(self, state, reward, isEnd, info):
        ...
        actor_lr_used = self.actor_lr_state if self.use_ach_lr_mod else self.actor_lr_base
...
def _resolve_config_path(cfg_arg: str) -> str:
    ...
    local = os.path.join(here, cfg_arg)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        "--config",
        required=False,
        default="config_ll_placecells_16k.ini",
    ...
    if adam:
        print("Note: forcing `adam = False` for this consolidated LunarLander script.")
    adam = False
    ...
    if input_type == 0:
        ...
        if env_name == "CartPole-v1":
            ...
            state_to_spike = State_to_spike_PlaceCell_LunarLander16k(env_dt, basis=basis, rep=rep)
        else:
            raise ValueError(f"Place-cell input_type=3 not implemented for {env_name}")
    else:
    ...
    for n in range(1 if test else n_run):
        ...
        if log_enabled and reward_history:
            run_tag = f"{n}_{name}_flagship_bind"
