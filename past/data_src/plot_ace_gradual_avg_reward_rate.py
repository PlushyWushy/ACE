#!/usr/bin/env python3
"""
Plot average reward rate (with std shading) across all `*_icarus` runs in sb_gradual.
...
OUT_PNG = Path("sb_gradual/ACE_avg_reward_rate.png")
OUT_STATS = Path("sb_gradual/ACE_reward_rate_stats.csv")
WINDOW = 50
...
def main():
    ...
    if not runs:
        print('No ACE (icarus) gradual runs found.')
        return
    ...
    # Gradual switch lines
    plt.axvline(x=10000, color='tab:red', linestyle='--', label='Switch Start')
    plt.axvline(x=10200, color='tab:orange', linestyle='--', label='Switch End')
    
    plt.xlabel('Episode')
    plt.ylabel('Percent Optimal (%)')
    plt.title('ACE - Average Reward Rate across seeds (±STD) - Gradual')
