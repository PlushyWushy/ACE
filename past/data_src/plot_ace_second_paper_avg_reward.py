#!/usr/bin/env python3
"""
Plot per-episode average reward (with std shading) across flagship runs in icarussecondpaper.
- Labeled as ACE in title and filenames.
"""
...
OUT_PNG = Path("icarussecondpaper/ACE_per_episode_avg_reward.png")
OUT_STATS = Path("icarussecondpaper/ACE_per_episode_reward_stats.csv")
...
def main():
    ...
    for d in sorted(RUNS_DIR.iterdir(), key=lambda p: p.name):
        if not d.is_dir() or not d.name.endswith('_flagship'):
            continue
    ...
    if not runs:
        print('No ACE (flagship) second paper runs found.')
        return
    ...
    plt.plot(episodes, mean, color='tab:blue', lw=2, label='Mean reward')
    plt.fill_between(episodes, mean - std, mean + std, color='tab:blue', alpha=0.25, label='±1 std')
    
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.title('ACE - Per-episode average reward across seeds (±STD) - Cartpole')
