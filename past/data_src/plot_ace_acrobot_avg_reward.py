#!/usr/bin/env python3
"""
Plot per-episode average reward (with std shading) across flagship runs in acrobot.
...
OUT_PNG = Path("acrobot/ACE_per_episode_avg_reward.png")
OUT_STATS = Path("acrobot/ACE_per_episode_reward_stats.csv")
...
def main():
    ...
    if not runs:
        print('No ACE (flagship) acrobot runs found.')
        return
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.title('ACE - Per-episode average reward across seeds (±STD) - Acrobot')
