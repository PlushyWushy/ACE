#!/usr/bin/env python3
"""
Plot per-episode average reward rate across classic multiswitch runs.
"""
...
OUT_PNG = Path("sb_multiswitch/classic_avg_reward_rate.png")
OUT_STATS = Path("sb_multiswitch/classic_reward_rate_stats.csv")
WINDOW = 50
...
def main():
    ...
    for d in sorted(RUNS_DIR.iterdir(), key=lambda p: p.name):
        if not d.is_dir() or 'classic' not in d.name:
            continue
    ...
    if not runs:
        print('No classic multiswitch runs found.')
        return
    ...
    # Switch points
    zplt.axvline(x=15000, color='tab:red', linestyle='--', label='Switch 1')
    ...
    plt.title('Classic - Multi-Switch Bandit Average Reward Rate (±STD)')
