#!/usr/bin/env python3
"""
Switch FrozenLake with decoupled uncertainty (Icarus variant):
...
State encoding: One-hot spike encoder (16 neurons for 4×4 grid).
"""
...
DT = 0.02
TAU_M = 0.02
ACTOR_THETA = 0.5    # Low threshold — with 1 active input, membrane ≈ single weight
GAMMA = 0.99         # High gamma to propagate sparse reward signal
...
# ---------------------------------------------------------------------------
# 1. One-Hot Encoder  (deterministic, discrete states)
# ---------------------------------------------------------------------------
class OneHotSpikeEncoder(nn.Module):
    """Deterministic one-hot encoder for discrete states.  Produces a clean
    one-hot vector — state i maps to neuron i = 1.0, all others = 0.0."""
...
def train(args):
    ...
    print(f"FrozenLake Icarus | Episodes: {args.episodes} | Switch at: {args.switch_ep}")
    ...
    # -----------------------------------------------------------------------
    out_dir = (f"frozenlake/runs/{args.seed}_flagship" if args.seed is not None
               else "frozenlake/runs/noseed_flagship")
    ...
    fig.suptitle("Switch FrozenLake — Icarus (Decoupled Uncertainty)", fontsize=14)
