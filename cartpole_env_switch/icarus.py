...
def train(episodes=4000, render=False, seed: int | None = None, shift_mode: str = DEFAULT_SHIFT_MODE):
    ...
    print(
        "Start Switch CartPole (NE+ACh -> Critic ACh Modulation + TD Fast/Slow Surprise). "
    ...
    fig.tight_layout()
    plt.title(
        f"CartPole - NE+ACh (Critic ACh Mod) + TD Surprise ({shift_label} switch) + SNN Critic"
    )

    safe_label = shift_label.replace(" ", "_")
    run_name = f"{seed}_{safe_label}_switch_fastslow" if seed is not None else f"noseed_{safe_label}_switch_fastslow"
