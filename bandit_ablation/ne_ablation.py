#!/usr/bin/env python3
"""
Switching bandit that blends the high-gain setup from `ne_ach.py` with the
logistic neuromodulator drives and fast/slow TD surprise traces from the
original `critic_ach.py`.
"""
...
# Logistic neuromodulator params (from critic_ach)
ACH_MAX = 1e-2
ACH_K = 3
# Lower center so moderate surprise already boosts LR/NE
ACH_CENTER = 0.9
...
# Surprise trace hyperparameters (from critic_ach)
...
def train(episodes: int = 4000, seed: int | None = SEED):
    ...
    plt.ylim(0, 105)
    plt.title("Switch Bandit - Logistic NE/ACh + TD Surprise Traces")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower right")
    plt.savefig("switch_bandit_rstdp_ne_ach_trace.png", dpi=150, bbox_inches="tight")
    print("\nPlot saved as 'switch_bandit_rstdp_ne_ach_trace.png'")
    print(f"Max Critic Scale: {max_critic_scale:.3f}")
    
    # Ensure output directory exists and save both CSV and PNG in bandit_ablation/runs
    out_dir = f"bandit_ablation/runs/{seed}_ne_ablation" if seed is not None else "bandit_ablation/runs/noseed_ne_ablation"
    os.makedirs(out_dir, exist_ok=True)

    png_path = os.path.join(out_dir, f"{seed}_ne_ablation.png")
    plt.savefig(png_path, dpi=150, bbox_inches='tight')

    csv_path = os.path.join(out_dir, f"{seed}_ne_ablation.csv")
