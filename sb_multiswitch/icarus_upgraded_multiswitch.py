...
def train(episodes: int = 10000, seed: int | None = SEED, num_switches: int = NUM_SWITCHES, n_arms: int = N_ARMS, **kwargs):
    ...
    plt.ylim(0, 105)
    plt.title(f"Multi-Switch Bandit ({n_arms} arms, {num_switches} switches)")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower right")
    
    out_dir = f"sb_multiswitch/runs/{seed}_icarus_multiswitch" if seed is not None else "sb_multiswitch/runs/noseed_icarus_multiswitch"
