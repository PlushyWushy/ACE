...
Actor LR is set directly by ACh (no decay), with expected surprise = critic variance.
"""
...
def train(args):
    ...
    # Expected uncertainty = critic variance (no EMA)
    avg_expected = 0.0
    ...
    for ep in range(args.episodes):
        ...
        while not done:
            ...
            avg_expected = var_val
            current_ach = logistic_drive(args.ach_max, ACH_K, ACH_CENTER, avg_expected, args.base_lr)

            actor_lr = current_ach
    ...
    fig.tight_layout()
    plt.title("Switch CartPole - Decoupled Uncertainty (TD-LTP Critic, Actor LR Direct)")

    out_dir = f"cartpole/runs/{args.seed}_critic_ach_decoupled_tdlp_actor_lr_direct_variance" if args.seed is not None else "cartpole/runs/noseed_critic_ach_decoupled_tdlp_actor_lr_direct_variance"
    os.makedirs(out_dir, exist_ok=True)

    png_path = os.path.join(out_dir, f"{args.seed}_critic_ach_decoupled_tdlp_actor_lr_direct_variance.png")
    fig.savefig(png_path, dpi=150, bbox_inches="tight")

    csv_path = os.path.join(out_dir, f"{args.seed}_critic_ach_decoupled_tdlp_actor_lr_direct_variance.csv")
