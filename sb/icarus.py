...
def logistic_drive(max_val: float, k: float, center: float, surprise: float, base: float) -> float:
    return max_val / (1.0 + math.exp(-k * (surprise - center))) + base
...
def train(episodes: int = 4000, seed: int | None = SEED):
    ...
    out_dir = f"switch_bandit_experiment/runs/{seed}_icarus" if seed is not None else "switch_bandit_experiment/runs/noseed_icarus"
    os.makedirs(out_dir, exist_ok=True)

    png_path = os.path.join(out_dir, f"{seed}_icarus.png")
    plt.savefig(png_path, dpi=150, bbox_inches='tight')

    csv_path = os.path.join(out_dir, f"{seed}_icarus.csv")
