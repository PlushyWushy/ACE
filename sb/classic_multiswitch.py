...
def _generate_switch_points(episodes: int, num_switches: int, n_arms: int = 2) -> Tuple[list, list]:
    ...
    Returns (switch_episodes, optimal_arm_sequence).
    """
...
def train(episodes: int = 10000, seed: int | None = SEED, num_switches: int = NUM_SWITCHES, n_arms: int = N_ARMS, **kwargs):
    ...
    out_dir = f"switch_bandit_experiment/runs/{seed}_classic_multiswitch" if seed is not None else "switch_bandit_experiment/runs/noseed_classic_multiswitch"
