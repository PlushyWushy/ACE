#!/usr/bin/env python3
"""
Multi-switch, multi-arm variant of icarus_upgraded.py:
...
def train(episodes: int = 10000, seed: int | None = SEED, num_switches: int = NUM_SWITCHES, n_arms: int = N_ARMS, **kwargs):
    ...
    optimal_arm = 0  # starts with arm 0 optimal
    ...
    # Use the saved copy for plotting (the original list was consumed by pop())
    ...
    plt.ylim(0, 105)
    plt.title(f"Multi-Switch Bandit ({n_arms} arms, {num_switches} switches) - Icarus Upgraded")
    plt.grid(True, alpha=0.3)
    plt.legend(loc="lower right")
    
    out_dir = f"switch_bandit_experiment/runs/{seed}_icarus_multiswitch" if seed is not None else "switch_bandit_experiment/runs/noseed_icarus_multiswitch"
    ...
    # Also save the switch points for reference
