...
OUT_PNG = BASE_DIR / "sb_normal/only_ach_avg_reward_rate.png"
WINDOW = 50
...
def main():
    ...
    for d in sorted(RUNS_DIR.iterdir()):
        if d.is_dir() and d.name.endswith('_only_ach'):
            csv_file = d / "data.csv"
    ...
    plt.plot(episodes, mean, color='tab:red', lw=2, label='Mean reward rate')
    plt.fill_between(episodes, mean - std, mean + std, color='tab:red', alpha=0.25, label='±1 std')
    plt.axvline(x=10000, color='blue', linestyle='--', alpha=0.5, label='Switch')
    plt.xlabel('Episode')
    plt.ylabel('Percent Optimal (%)')
    plt.title('Only ACh Modulation - Average Reward Rate across seeds (±STD)')
